...
var Popover = React.createClass({
  ...
  computeGeometry({ contentSize, placement }, fromRect) {
    ...
    var displayArea;
    if (
      fromRect.SCREEN_WIDTH != undefined &&
      fromRect.SCREEN_HEIGHT != undefined
    ) {
      displayArea = new Rect(
        10,
        10,
        fromRect.SCREEN_WIDTH - 20,
        fromRect.SCREEN_HEIGHT - 20
      );
    } else {
  ...
  getTranslateOrigin() {
    ...
    var popoverCenter = new Point(
      popoverOrigin.x + contentSize.width / 2,
      popoverOrigin.y + contentSize.height / 2
    );
    return new Point(
      this.getPolarity() * (anchorPoint.x - popoverCenter.x),
  ...
  componentWillReceiveProps(nextProps: any) {
    ...
    var { isVisible, fromRect } = this.props;
    ...
    if (this.props.mode === "select") {
      ...
      contentStyle =
        this.props.title == null
          ? [styles.popoverContent, styles.popoverTopRadius]
          : styles.popoverContent;

      if (placement === PLACEMENT_OPTIONS.TOP) {
        arrowColorStyle = this.getArrowColorStyle(
          flattenStyle(styles.title).backgroundColor
        );
      } else {
        arrowColorStyle = this.getArrowColorStyle(
          flattenStyle(styles.popoverContent).backgroundColor
        );
      }
    }
        ...
        >
          ...
          >
            <Animated.View
              ...
              style={[contentContainerStyle, contentModeStyling]}
            >
              {this.props.title !== null && this.props.title !== undefined ? (
                <View
                  style={[
                    titleStyle,
                    { width: contentSizeAvailable },
                    dropShadowStyling
                  ]}
                >
